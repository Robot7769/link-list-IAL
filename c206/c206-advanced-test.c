...
int MaxListLength2 = 100;                 /* for hangling wrongly linked lists */
...
/* Prints elements and activity of list TL. */
	...
	printf("-----------------");
	while ((TempList.firstElement!=NULL) && (CurrListLength<MaxListLength2))	{
...
int test_DLL_GetFirst()	{
	...
	if (!solved)	{
		...
		if (error_flag)	{
			printf("Function DLL_GetFirst has called function DLError.\n");
			error_flag=FALSE;
			return(FALSE);
		}	
		else	{	 			
...
int test_DLL_GetLast()	{
	...
	if (!solved)	{
		...
		if (error_flag)	{
			printf("Function DLL_GetLast has called function DLError.\n");
			error_flag=FALSE;
			return(FALSE);
		}	
		else	{	 			
...
int test_DLL_GetValue()	{
  ...
  DLL_GetValue(&TEMPLIST,&temp);	
	...
	if (!solved)	{
		...
		if (error_flag)	{
			printf("Function DLL_GetValue has called function DLError.\n");
			error_flag=FALSE;
			return(FALSE);
		}	
		else	{	 			
